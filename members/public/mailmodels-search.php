...
$term = trim(strip_tags($_GET['idref'])); 
...
if ($row) {
	while ($i < $countrows){
		...
		$id = stripslashes($row[$i]['tm_idmodelocarta']);
		$nombre = stripslashes($row[$i]['tm_nombre']);
